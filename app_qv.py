# Generating Answers using Query-Visualisation Embedding
...
looker_base_embed_url = "https://demo.looker.com/embed/explore/thelook/order_items?" #### Separting this to avoid having issues with 
...
# Function for Getting QID and SLUG
def parse_input_url(input_url):
    ...
    #search_info, output
    return query_id, query_viz, output
...
def search(query):
    ...
    if save_logs(query, response, "app_qv/all_logs.jsonl"):
            print("Logs Saved Correctly")  
    url = f"{looker_base_embed_url}{response}&toggle=dat,pik,vis&allow_login_screen=true"

    qid,slug_viz_url,query_json = parse_input_url(url)
    slug_viz_url = os.environ['LOOKERSDK_BASE_URL']+slug_viz_url
    
    return (f'<iframe src="{slug_viz_url}" style="width:100%; height:600px;"></iframe>', qid, query_json, url)
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        ...
        with gr.Tab(label="Info"):
            with gr.Row():
                qid_text = gr.Text(value="qid", label="Query ID")
            with gr.Row():
                #query_json = gr.Text(value="query_json", label="Looker Query")
                query_json = gr.JSON()
            with gr.Row():
                extended_url = gr.Text(value="url", label="Looker Extended URL")
        search_input.submit(search, inputs=search_input, outputs=[search_results, qid_text, query_json, extended_url])
