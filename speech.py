...
audio_input = gr.Audio(source="microphone", format="mp3", type="filepath")
output_text = gr.HTML()
...
#    with gr.Row():
#        audio_input = gr.Audio(source="microphone", format="mp3", type="filepath")
#        c = gr.Textbox(label="Type your question and press Enter", lines=1, placeholder="Sales this week ?")
#        audio_input.change(extract_text_uri, inputs=audio_input, outputs=c)
#    with gr.Row():
#        search_results = gr.HTML()
#        #c.change(search, inputs="Whats my sales htis week", outputs=search_results)
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale = 8):
            search_bar = gr.Textbox(label="Search", placeholder="Record your question first", show_label=True, interactive=False)
            input.change(fn=extract_text_uri, inputs=input, outputs=search_bar)
    with gr.Row():
        #gr.Markdown("logic for search")
        iframe = gr.HTML()
        search_bar.change(fn=search, inputs=search_bar, outputs=iframe)
